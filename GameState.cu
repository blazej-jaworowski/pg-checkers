...
void GameState::add_man_non_take_moves(uint8_t pos)
{
    ...
    if (black_turn)
    {
        if (se < 32 && empty_at(se))
            valid_moves[valid_move_count++] = create_move(pos, se);
        if (sw < 32 && empty_at(sw))
            valid_moves[valid_move_count++] = create_move(pos, sw);
    }
    else
    {
        if (ne < 32 && empty_at(ne))
            valid_moves[valid_move_count++] = create_move(pos, ne);
        if (nw < 32 && empty_at(nw))
            valid_moves[valid_move_count++] = create_move(pos, nw);
    }
}
...
int8_t GameState::simulate_game() const
{
    ...
    return simulate_game(rng);
}

int8_t GameState::simulate_game(thrust::random::minstd_rand &rng) const
{
    GameState copy(*this);
    while (!copy.finished)
    {
        thrust::random::uniform_int_distribution<uint8_t> dist(0, copy.valid_move_count - 1);
        uint16_t move = copy.valid_moves[dist(rng)];
        copy.play_move(move);
