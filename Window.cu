...
Window::Window(int width, int height) : width(width), height(height)
{
    ...
    if (custom_event_type == (Uint32)-1)
        print_error("Error registering custom event");
    ...
    if (renderer == nullptr)
        print_error("Error creating renderer");
...
SDL_Texture *Window::load_texture(const std::string &filename)
{
    ...
    if (surface == nullptr)
        print_error("Error creating surface from BMP");
    SDL_Texture *r = SDL_CreateTextureFromSurface(renderer, surface);
    if (r == nullptr)
        print_error("Error creating texture");
